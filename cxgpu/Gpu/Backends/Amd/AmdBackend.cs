...
internal sealed class AmdBackend : GpuBackendPlugin
{
    ...
    /// order. Set from <c>CXGPU_AMD_READER</c>; the user-facing plugin setting will feed the same
    ...
    private string? ForcedMechanism
    {
        get
        {
            ...
            var configured = _readerSetting ?? Environment.GetEnvironmentVariable("CXGPU_AMD_READER");
    ...
    /// installed BOTH work, so this is a real choice the user can make and cxgpu itself cannot —
